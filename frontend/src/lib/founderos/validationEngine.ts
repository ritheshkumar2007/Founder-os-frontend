import type { IdeaScore, ValidationState, Venture } from "./types";
...
export interface ValidationTurnResult {
  reply: string;
...
export function processValidationTurn({
  userMessage,
  validationState,
  venture,
}: {
  ...
  if (state.currentQuestion === 5) {
    ...
    const scoreData: IdeaScore = {
      ...
      strengths: [problemReason, painReason],
      risks: hasZeroEvidence ? [evidenceReason] : [],
      recommendations: ["Scope precision MVP to test customer activation."],
      interviewMultiplier: totalScore >= 80 ? 1.2 : 1.0,
