...
export const SprintMissionAnalysisModal: React.FC<SprintMissionAnalysisModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-[#020408]/85 backdrop-blur-xl animate-fade-in select-none">
      <div className="w-full max-w-2xl rounded-2xl border border-[rgba(139,92,246,0.4)] bg-[#0b0f12] text-[#e0e3e7] shadow-[0_25px_60px_rgba(0,0,0,0.8),0_0_35px_rgba(139,92,246,0.2)] overflow-hidden flex flex-col max-h-[90dvh]">
        {/* Modal Header */}
          ...
          <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] sm:text-xs font-mono text-[#cbc3d7]/70 truncate">
                Target: {ventureName || "Active Venture"}
              </p>
            </div>
          </div>
          <button
        ...
        <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 overflow-y-auto flex-1">
          {/* Progress / Status */}
          <div className="p-3.5 sm:p-4 rounded-xl bg-[#101417] border border-[rgba(139,92,246,0.3)] space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] sm:text-xs font-mono font-bold text-white truncate">
                  {analyzing ? "COMPUTING TELEMETRY..." : "ANALYSIS LOCKED & READY"}
                </span>
              </div>
          ...
          <div className="rounded-xl border border-[rgba(139,92,246,0.3)] bg-[#020408] p-3 sm:p-4 font-mono text-[11px] sm:text-xs text-[#cbc3d7] space-y-1.5 h-32 sm:h-36 overflow-y-auto shadow-inner">
          ...
          {completed && (
            <div className="space-y-4 animate-fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 sm:gap-3 text-xs font-mono">
                <div className="p-3 sm:p-3.5 rounded-xl border border-[rgba(139,92,246,0.3)] bg-[#101417]">
                  <span className="text-[#958ea0] uppercase text-[10px] block">Velocity Index</span>
                  <span className="text-lg sm:text-xl font-bold text-white">{velocityScore}%</span>
                  <span className="text-[10px] sm:text-[11px] text-[#A78BFA] block mt-0.5">Top 10% pacing</span>
                </div>
                <div className="p-3 sm:p-3.5 rounded-xl border border-[rgba(139,92,246,0.3)] bg-[#101417]">
                  <span className="text-[#958ea0] uppercase text-[10px] block">Directives Done</span>
                  <span className="text-lg sm:text-xl font-bold text-white">{completedDirectivesCount} / {totalDirectivesCount}</span>
                  <span className="text-[10px] sm:text-[11px] text-[#A78BFA] block mt-0.5">Core targets locked</span>
                </div>
                <div className="p-3 sm:p-3.5 rounded-xl border border-[rgba(139,92,246,0.3)] bg-[#101417]">
                  <span className="text-[#958ea0] uppercase text-[10px] block">Days Remaining</span>
                  <span className="text-lg sm:text-xl font-bold text-white">{Math.max(1, 7 - activeDay)} Days</span>
                  <span className="text-[10px] sm:text-[11px] text-[#cbc3d7] block mt-0.5">T-Minus to Launch</span>
                </div>
              </div>

              {/* Actionable Sprint Directives */}
              <div className="p-3.5 sm:p-4 rounded-xl border border-white/10 bg-[#101417] space-y-2.5 text-xs">
        ...
        {/* Modal Footer */}
        <div className="px-4 sm:px-6 py-3.5 sm:py-4 border-t border-[rgba(139,92,246,0.25)] bg-[#101417] flex flex-col sm:flex-row justify-between items-center gap-2.5 text-xs font-mono">
          <button
          ...
          <button
            onClick={onClose}
            className="w-full sm:w-auto bg-[#A78BFA] hover:bg-[#bfa8ff] text-black font-bold px-5 py-2.5 rounded-lg transition-all shadow-[0_0_15px_rgba(139,92,246,0.3)] cursor-pointer"
          >
