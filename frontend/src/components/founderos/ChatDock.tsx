    ...
    <>
      ...
      {!open && (
        <button
          ...
          className="fixed bottom-6 right-6 z-40 flex items-center gap-2.5 rounded-full border border-white/10 bg-[#0b0f12] px-5 py-3 text-xs font-bold text-white shadow-[0_4px_20px_rgba(0,0,0,0.5)] transition-all hover:scale-105 hover:border-white/30 active:scale-95 cursor-pointer"
        >
          <span className="relative flex size-2.5">
            <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-zinc-800 opacity-75" />
            <span className="relative inline-flex size-2.5 rounded-full bg-[#71717a]" />
          </span>
      ...
      {open && (
        <div className="fixed bottom-6 right-6 z-50 flex h-[620px] w-[420px] max-w-[calc(100vw-2rem)] flex-col overflow-hidden rounded-2xl border border-white/10 bg-[#0b0f12] shadow-[0_25px_60px_rgba(0,0,0,0.8)] backdrop-blur-2xl os-window-open">
          {/* Header */}
          <div className="flex items-center justify-between border-b border-white/10 bg-[#101417] px-5 py-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="rounded-full bg-zinc-800/60 px-2 py-0.5 font-mono text-[9px] font-bold text-zinc-300 border border-white/10">
                    Active Context
                  </span>
                </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-full text-center p-6 space-y-3">
                <div className="grid size-12 place-items-center rounded-2xl border border-white/10 bg-zinc-800/60 text-zinc-300">
                ...
                <p className="text-xs text-[#cbc3d7] max-w-[260px]">
                  Ask questions about <span className="text-zinc-300 font-semibold">{page}</span> or request strategic startup advice.
                </p>
              </div>
            ) : (
              messages.map((m) => (
                ...
                >
                  <div
                    className={cn(
                      ...
                      m.role === "user"
                        ? "bg-zinc-800 text-white border border-white/10 font-medium shadow-[0_4px_20px_rgba(0,0,0,0.5)]"
            ...
            {busy && (
              <div className="flex items-center gap-2 text-xs text-zinc-300 font-mono bg-zinc-800/60 px-3.5 py-2.5 rounded-xl border border-white/10 w-fit">
                <Sparkles className="size-3.5 animate-spin text-zinc-300" />
          ...
          <div className="border-t border-white/5 bg-[#101417]/50 p-2.5 overflow-x-auto no-scrollbar flex gap-2">
            {suggestions.map((s, idx) => (
              <button
                ...
                disabled={busy}
                className="shrink-0 rounded-lg border border-white/10 bg-[#101417] px-2.5 py-1 text-[11px] font-medium text-[#cbc3d7] transition hover:border-white/10 hover:text-white hover:bg-zinc-800/60 disabled:opacity-50 cursor-pointer"
              >
                {s}
              </button>
            ))}
          ...
          {/* Input Box */}
          <div className="border-t border-white/10 bg-[#101417] p-3">
            <form
              onSubmit={(e) => {
                e.preventDefault();
                void handleSend();
              }}
              className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#0b0f12] p-2 focus-within:border-[#d4d4d8]"
            >
              <textarea
                ref={inputRef}
                rows={1}
              ...
              <button
                ...
                className="grid size-8 place-items-center rounded-lg btn-system text-white transition disabled:opacity-40 cursor-pointer"
              >
