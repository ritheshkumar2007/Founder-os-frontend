...
export const TechMarquee: React.FC = () => {
  return (
    <section className="relative py-12 border-y border-white/[0.08] bg-[#02040A]/90 overflow-hidden z-10">
      {/* Title */}
      <div className="text-center mb-6">
        <p className="text-[11px] font-mono text-[#74839A] uppercase tracking-[0.25em]">
          ...
          {[...PARTNERS, ...PARTNERS].map((partner, index) => {
            ...
            return (
              <div
                ...
                className="flex items-center gap-3 px-5 py-2.5 rounded-xl border border-white/[0.08] bg-[#07111F]/60 backdrop-blur-md text-xs font-mono transition-all hover:border-[#4F8CFF]/40 hover:bg-[#0B1628]"
              >
                <Icon className="size-4 text-[#4F8CFF]" />
                <span className="font-bold text-white tracking-wider">{partner.name}</span>
                <span className="text-white/20">•</span>
                <span className="text-[#AAB7CC] text-[11px]">{partner.role}</span>
              </div>
            );
          })}
        </div>
